...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      show() {
        this.showFlag = true
        setTimeout(() => { // 在dom渲染了以后才能正确计算高度，所以需要重新refresh
      ...
      scrollToCurrent(current) {
        const index = this.sequenceList.findIndex((song) => { // 找到当前歌曲在列表中的索引
          return current.id === song.id
        })
        this.$refs.listContent.scrollToElement(this.$refs.listItem[index], 300) // 滚动到当前歌曲 300ms的动画
      },
      deleteOne(item) {
